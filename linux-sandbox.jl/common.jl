...
function check_configs(brgs::Vector{BuildkiteRunnerGroup})
    ...
    # If we are pinning cores, we need to create cgroups for each.  Note that this
    # `mk_cgroup` machinery is only used by `debug_shell()` nowadays; the systemd
    # services are pinned through `AllowedCPUs=` drop-ins instead, as generated by
    # `generate_cpu_pinning_dropins()`.
    if pinned_cores > 0
...
function generate_systemd_script(io::IO, brg::BuildkiteRunnerGroup; agent_name::String=string(brg.name, "-%i"), kwargs...)
    ...
    with_executor(UnprivilegedUserNamespacesExecutor) do exe
        ...
        # Note: CPU pinning of the agents (if `num_cpus > 0`) is not handled here,
        # but rather through per-instance systemd drop-ins setting `AllowedCPUs=`;
        # see `generate_cpu_pinning_dropins()`.
...
# Pin each agent's service (and everything it spawns, including `dockerd` and the
# sandbox itself) to its assigned set of CPUs.  We do this with per-instance systemd
# drop-ins rather than the `mk_cgroup` machinery, since the latter relies on the
# delegated `user@.service` cgroup subtree, which is only available to user services
    ...
    for brg in brgs
        for _ in 1:brg.num_agents
            ...
            if brg.num_cpus > 0
                ...
                dropin_dir = joinpath(systemd_system_dir, "$(unit_name).service.d")
                run(`sudo mkdir -p $(dropin_dir)`)
                sudo_write(joinpath(dropin_dir, "50-cpu-pinning.conf"), """
                    [Service]
                    AllowedCPUs=$(cpus)
                    CPUAffinity=$(cpus)
                    """)
